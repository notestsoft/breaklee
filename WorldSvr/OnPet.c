...
CLIENT_PROCEDURE_BINDING(PET_COMMAND) {
	...
	S2C_DATA_PET_COMMAND* Response = PacketBufferInit(Connection->PacketBuffer, S2C, PET_COMMAND);
	...
	if (Packet->CommandType == CSC_DATA_PET_COMMAND_TYPE_INFO) {
		...
		S2C_DATA_PET_COMMAND_INFO* ResponseData = PacketBufferAppendStruct(Connection->PacketBuffer, S2C_DATA_PET_COMMAND_INFO);
	...
	if (Packet->CommandType == CSC_DATA_PET_COMMAND_TYPE_UNSEAL) {
		...
		S2C_DATA_PET_COMMAND_UNSEAL* ResponseData = PacketBufferAppendStruct(Connection->PacketBuffer, S2C_DATA_PET_COMMAND_UNSEAL);
	...
	if (Packet->CommandType == CSC_DATA_PET_COMMAND_TYPE_TRAIN) {
		...
		S2C_DATA_PET_COMMAND_TRAIN* ResponseData = PacketBufferAppendStruct(Connection->PacketBuffer, S2C_DATA_PET_COMMAND_TRAIN);
