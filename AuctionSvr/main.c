...
#define IPC_W2A_COMMAND(__NAME__) IPC_COMMAND_CALLBACK(W2A, __NAME__)
...
Int32 main(Int32 ArgumentCount, CString* Arguments) {
    CString ConfigFileName = "AuctionSvr.ini";
    if (ArgumentCount > 1) {
    ...
    NodeID.Group = Config.AuctionSvr.GroupIndex;
    NodeID.Index = 0;
...
#define IPC_W2A_COMMAND(__NAME__) \
    Trace("RegisterCommand(%d, IPC_W2A_%s)", IPC_W2A_ ## __NAME__, #__NAME__); \
    IPCSocketRegisterCommandCallback(Server->IPCSocket, IPC_W2A_ ## __NAME__, &SERVER_IPC_W2A_PROC_ ## __NAME__);
#include "IPCCommands.h"
