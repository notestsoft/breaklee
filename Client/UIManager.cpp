...
CUIInputState::CUIInputState() :
    ...
    memset(PressedKeyStates, 0, sizeof(PressedKeyStates));
}
...
CUIManager::CUIManager(
    ...
    Root(new CUIControl(this, 0, "", nullptr)) {
    Root->Active = true;
...
Bool CUIManager::LoadData(
    ...
    Char Name[1024];
    for (UInt Index = 0; Index < TextureCount; Index += 1) {
        if (Cursor + sizeof(UInt16) > End) return false;
        ...
        CTexture Texture = Renderer->LoadTexture(FilePath, false);
        CUIImage Image(String(Name), Width, Height, CUITexture(Texture, CUIRect(0, 0, Width, Height)));
    ...
    for (UInt Index = 0; Index < ColorCount; Index += 1) {
    ...
    for (UInt Index = 0; Index < ImageCount; Index += 1) {
        ...
        CUIImage Sprite(Image->Name, Image->Width, Image->Height, Image->Texture.Slice(X, Y, Width, Height));
    ...
    for (UInt Index = 0; Index < ElementCount; Index += 1) {
        if (Cursor + sizeof(UInt16) > End) return false;
        UInt16 NameLength = *(UInt16*)Cursor;
        Cursor += sizeof(UInt16);

        if (Cursor + NameLength > End) return false;
        memcpy(Name, Cursor, NameLength);
        Name[NameLength] = '\0';
        Cursor += NameLength;
        ...
        CArray<CUIEvent*> Events;
        for (Int Offset = 0; Offset < EventCount; Offset += 1) {
            Events.push_back(&this->Events[EventIndex + Offset]);
        }

        CArray<CUIImage*> Sprites;
        for (Int Offset = 0; Offset < SpriteCount; Offset += 1) {
            Sprites.push_back(&this->Sprites[SpriteIndex + Offset]);
        }

        CArray<CUILabel*> Labels;
        for (Int Offset = 0; Offset < LabelCount; Offset += 1) {
    ...
    this->SetEventHandler(2, [](CUIManager* Manager, CUIElement* Source, CUIEvent* Event) {
       /*
        AllocatorRef Allocator = AllocatorGetCurrentDefault();
        SocketRef Socket = SocketCreate(
            Allocator,
            ...
            true,
            LoginSocketOnConnect,
            LoginSocketOnDisconnect,
            LoginSocketOnSend,
            LoginSocketOnReceived,
            Application
        );
        */
        // TODO: Login event
...
Void CUIManager::BuildControl(
    ...
    if (Element->Type == CUIElementType::Container) {
        ...
        auto Container = new CUIContainer(this, Element->ID, Element->Name, Element);
        Control = Container;
    ...
    if (Element->Type == CUIElementType::Button) {
        auto Button = new CUIButton(this, Element->ID, Element->Name, Element);
        Control = Button;
        ...
        for (auto Label : Element->Labels) {
            ...
            auto TextBox = new CUITextBox(this, Label->ID, Label->Name, nullptr);
    ...
    if (Element->Type == CUIElementType::EditBox) {
        ...
        auto EditBox = new CUIEditBox(this, Element->ID, Element->Name, Element);
        Control = EditBox;

        assert(Element->Sprites.size() == 4); // TODO: Find out what the textures are used for
        EditBox->CaretTexture = Element->Sprites[0]->Texture;
        EditBox->UnknownTexture1 = Element->Sprites[1]->Texture;
        EditBox->UnknownTexture2 = Element->Sprites[2]->Texture;
        EditBox->UnknownTexture3 = Element->Sprites[3]->Texture;
    ...
    if (Element->Type == CUIElementType::GridBox) {
        ...
        auto GridBox = new CUIGridBox(this, Element->ID, Element->Name, Element);
        Control = GridBox;
    ...
    if (Element->Type == CUIElementType::TextBox) {
        ...
        auto TextBoxGroup = new CUITextBoxGroup(this, Element->ID, Element->Name, Element);
    ...
    if (Element->Type == CUIElementType::ProgressBar) {
        ...
        auto ProgressBar = new CUIProgressBar(this, Element->ID, Element->Name, Element);
        Control = ProgressBar;
    ...
    if (Element->Type == CUIElementType::Sprite) {
        // assert(Element->Events.size() == 0);
        assert(Element->Labels.size() == 0);
        auto Sprite = new CUISprite(this, Element->ID, Element->Name, Element);
        Control = Sprite;
    ...
    if (Element->Type == CUIElementType::CheckBox) {
        auto CheckBox = new CUICheckBox(this, Element->ID, Element->Name, Element);
        Control = CheckBox;
    ...
    //    Element->Rect
        ...
        auto SliderBar = new CUISliderBar(this, Element->ID, Element->Name, Element);
        Control = SliderBar;
    ...
    if (Element->Type == CUIElementType::Tree) {
        ...
        auto Tree = new CUITree(this, Element->ID, Element->Name, Element);
        Control = Tree;
    ...
    if (Element->Type == CUIElementType::Tri) {
        auto Tri = new CUIControl(this, Element->ID, Element->Name, Element);
        Control = Tri;
    }
    ...
    if (Element->Type == CUIElementType::ScrollBar) {
        ...
        auto ScrollBar = new CUIScrollBar(this, Element->ID, Element->Name, Element);
        Control = ScrollBar;
    ...
    if (Element->Type == CUIElementType::TabPanel) {
        auto TabPanel = new CUITabPanel(this, Element->ID, Element->Name, Element);
        Control = TabPanel;
    }

    if (Element->Type == CUIElementType::Mtl) {
        auto Mtl = new CUIControl(this, Element->ID, Element->Name, Element);
        Control = Mtl;
    }

    if (Element->Type == CUIElementType::CobTab) {
        auto CobTab = new CUIControl(this, Element->ID, Element->Name, Element);
        Control = CobTab;
    }

    if (Element->Type == CUIElementType::Unk) {
        auto Unk = new CUIControl(this, Element->ID, Element->Name, Element);
        Control = Unk;
    }

    if (Element->Type == CUIElementType::GaugeBar) {
        auto GaugeBar = new CUIGaugeBar(this, Element->ID, Element->Name, Element);
        Control = GaugeBar;
    }
