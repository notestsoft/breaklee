...
STATEMENT(
    ...
    "SELECT `account_id`, `session_ip`, `session_timeout`, `character_slot_id`, `character_slot_order`, `character_slot_flags`, `character_password`, `character_question`, `character_answer`, `created_at`, `updated_at` FROM `accounts` WHERE `account_id` = ?;"
)
...
STATEMENT(
    ...
    "SELECT `characters`.`account_id`, `characters`.`id`, `characters`.`name`, `characters`.`index`, `info`.`data` AS `info_data`, `equipment`.`data` AS `equipment_data`, `characters`.`created_at`, `characters`.`updated_at` "
    "FROM `characters` "
